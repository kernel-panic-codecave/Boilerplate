...
2. **Warehouse bounds (M3)**: a `WarehousePlannerItem` defines the storage volume by right-clicking two corners, then binding to the controller. No literal built shell required.
...
6. **Recipe viewer support (M6)**: all three of JEI, REI, and EMI, not REI alone - see `m6-polish-parity.md`'s "Recipe-viewer integration" section for the resulting dependency/plugin-shim shape.
