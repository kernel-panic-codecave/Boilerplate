 ...
 * its two surfaces.
 *
 * **Client-only.** A dedicated server never reads [net.kernelpanicsoft.boilerplate.resource.ResourceKind.display],
...
object ItemDisplayKind : ResourceDisplayKind {
	...
	override fun worldMesh(resource: ResourceComponent, amount: Long): Mesh? =
		(resource as? ItemResource)?.takeIf { !it.isBlank }?.let { InstancedMeshes.itemMesh(it) }
	...
	override fun tooltipLines(resource: ResourceComponent, amount: Long): List<Component> {
		...
		val minecraft = requireMinecraftClient
