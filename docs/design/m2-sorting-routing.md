...
`InterfaceHookType` (see "Hook-to-hook facing as a subnet boundary" below) doesn't fit this grid at all - it's neither a source nor a sink in the push/pull sense, just a passive physical buffer other hooks (on either side of a subnet boundary) read from or write to directly.
...
**`InterfaceHookType`/`InterfaceHookState`**: a passive stock buffer - `val stock: ArchieItemStorage by itemField(9)` - exposed to `ItemApi.BLOCK` on its own face via `TileRegistry.Hook`'s direction-gated `exposeItemStorage`, so anything physically touching it (a hopper, another mod's pipe, or another Tubular Storage hook facing it directly) inserts/extracts exactly like it would against any ordinary inventory - "normal interaction is like the ME interface." Has its own `hasMenu`/`InterfaceHookMenu` (a plain 3x3 `Slots` grid, chest-simple) for direct player access too. A non-blank slot's own target is just its ordinary `ArchieItemSlot.getLimit` (a full stack of whatever occupies it) - no separate configured-amount field, the same "put a stack of what you want" simplicity `RequesterHookState.request` already uses, just physically held here instead of read off an adjacent inventory. Never initiates anything on its own tick - purely a passive target, unlike the active hooks below.
...
- `PipeRouter.step` (push-routing BFS) - a boundary edge is evaluated as a normal candidate (against whatever `current`'s own attached hook is) instead of enqueued for further walking, so the far network's topology beyond the interface's own stock never becomes visible to this search.
...
- **`FilterHookType`** - insert-only. Already a normal `validRoute` push candidate; the only missing piece was `PipeRouter.step` not folding a boundary edge into its BFS pass-through (see above) - once that's fixed, the existing `SortingHookState` filter/mode candidate check runs against the interface's own stock exactly like any other destination.
- **`SyncHookType`** - filtered two-way, simply because it's already both a push candidate (`validRoute`) and a provider source (`providesItems`) on the *same* network today - facing an interface just means both of those now cross the seam instead of staying local.
...
- **`RequesterHookType`** - flips this hook's whole role from "request for myself" to "keep the interface's own stock topped up": `tryRequest` detects a boundary partner via `SubnetBoundary.interfaceAt` and, if found, iterates every non-blank slot in `InterfaceHookState.stock` computing its own shortfall (`getLimit - amount`) and issuing one `RequestFulfillment.request` per slot, instead of reading its single `RequesterHookState.request` order.
