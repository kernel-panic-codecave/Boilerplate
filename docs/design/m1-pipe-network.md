...
- **`HookBlockEntity : PipeBlockEntity`** (backs `HookBlock`):
  ...
  class HookBlockEntity(pos: BlockPos, state: BlockState) : PipeBlockEntity(TileRegistry.Hook, pos, state), ExtendedMenuProvider {
      @Sync val hooks by mapField(HookState.serializer()) { emptyMap() }   // keyed by Direction.name
      val filterNorth by itemField(9)   // ...and filterSouth/East/West/Up/Down
      var pipeBlockId: ResourceLocation by field(ResourceLocationSerializer) { BuiltInRegistries.BLOCK.getKey(BlockRegistry.Pipe) }
  }
  ```
  `pipeBlockId` remembers which pipe type this hook block was promoted from (or defaults to the plain pipe for a fresh placement) - see Rendering below.

### Hooks: attachments, not separate blocks

A `HookBlockEntity` carries zero or more **hooks** — one per face, each an attachment rather than a distinct block, so a single pipe segment can carry an extraction hook on one face and a sorting hook on another simultaneously. `PipeHookType` is the behavior for a *kind* of hook (extraction, sorting, and whatever M3+ adds - a valve, a gauge); `HookState` is the small per-face persisted payload stored in `HookBlockEntity.hooks`, keyed by `Direction.name` - see [m2-sorting-routing.md](m2-sorting-routing.md#module-data) for its current shape.
...
- `HookState`'s fields (`type`/`routing`/`ticksSinceExtraction`) are one shared shape reused - and partly ignored - by every hook kind; making each `PipeHookType`'s state genuinely self-contained (its own `NBTHolder`-backed type, nested inside `HookBlockEntity.hooks` rather than one flat data class) is a real improvement, but needs new "nested NBTHolder" support in Archie that doesn't exist yet. Scoped out of the `HookBlock` split to keep that change landable; a dedicated follow-up.
- Two Archie `@Sync` bugs found while building this, now fixed upstream in Archie: (1) `NBTHolder.mapField`/`listField` registered the wrong serializer for `@Sync` (element instead of collection), crashing on sync; (2) `BlockEntityStateContainer`'s reflective property scan used module-less, generics-erased `KClass.serializerOrNull()`, which can't resolve a `@Contextual`-annotated field at all. Both fixed, which is what let `HookState.routing`/`hooks` fold back into one `@Sync`-annotated map (see [m2-sorting-routing.md](m2-sorting-routing.md#module-data)) instead of the separate-field workaround this doc originally described.
- `hooks` is both `@Sync` (for `BlockEntityStateManager`, which only pushes to a player with a menu open) *and* explicitly resynced via `level.sendBlockUpdated(...)` at the point of mutation (see `HookBlock.useItemOn`/`useWithoutItem`, `HookItem.place`) - the latter is what every *nearby* client's own block entity copy actually needs, since that's what `PipeHookBlockEntityRenderer` reads, and `@Sync` alone doesn't reach it.
