 ...
 * `docs/design/m6-polish-parity.md`) - this file and `neoforge`'s own copy are near-duplicates by
 * necessity, each compiled against that loader's own `:api` jar. Discovery differs per loader too:
 * Fabric finds this class via the `"emi"` entrypoint declared in `fabric.mod.json`; NeoForge's own
 * copy is annotated `@EmiEntrypoint` instead.
 *
