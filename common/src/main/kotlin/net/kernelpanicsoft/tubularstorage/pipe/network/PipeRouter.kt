 ...
 * `(networkId, network.version, resource, exclude)` and invalidated automatically whenever the
 * network's topology (or, from M2 onward, its routing modules) changes.
 */
object PipeRouter {
	private data class CacheKey(val networkId: UUID, val version: Int, val resource: ItemResource, val exclude: BlockPos?)
	 ...
	 * it came from.
	 */
	fun findRoute(level: ServerLevel, from: BlockPos, resource: ItemResource, exclude: BlockPos? = null): List<BlockPos>? {
		...
		val key = CacheKey(networkId, network.version, resource, exclude)
		cache[key]?.let { return it }
		if (cache.containsKey(key)) return null // cached miss

		val route = search(level, from, resource, exclude)
		cache[key] = route
		return route
	}

	private fun search(level: ServerLevel, from: BlockPos, resource: ItemResource, exclude: BlockPos?): List<BlockPos>? {
