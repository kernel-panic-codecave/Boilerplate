...
import net.kernelpanicsoft.tubularstorage.network.UpdateFilterCardModePacket
...
/**
 * Editor for one [net.kernelpanicsoft.tubularstorage.pipe.hook.filter.FilterCardItem] stack: its
 * whitelist/blacklist mode, plus whichever fields the selected
 * [net.kernelpanicsoft.tubularstorage.pipe.hook.filter.FilterConditionType] itself renders via its
 * own [net.kernelpanicsoft.tubularstorage.pipe.hook.filter.FilterConditionType.Content] - this
 * screen dispatches to it generically via [FilterCardMenu.type], the same reasoning
 * [net.kernelpanicsoft.tubularstorage.pipe.hook.filter.FilterCardState.matches] already dispatches
 ...
 * [net.kernelpanicsoft.tubularstorage.pipe.hook.filter.pushFieldUpdate]. [type] itself is fixed by
 * which item this is (see [net.kernelpanicsoft.tubularstorage.pipe.hook.filter.FilterCardItem]'s
			...
			ContainerPanel(contentWidth = contentWidth) {
				Column(verticalArrangement = Arrangement.spacedBy(6)) {
					...
					RadioGroup(
						...
						onSelected = {
							mode = it
							TubularStorageNetworkChannel.toServer(UpdateFilterCardModePacket(it))
						},
					)
