...
import net.kernelpanicsoft.boilerplate.pipe.block.PartBlock
...
/**
 * A kind of attachment a [net.kernelpanicsoft.boilerplate.pipe.entity.MultipartBlockEntity]
 * can carry - the shared base behind [net.kernelpanicsoft.boilerplate.pipe.hook.PipeHookType]
 * (one per face) and [net.kernelpanicsoft.boilerplate.pipe.encasement.PipeEncasementType] (the
 * whole-segment wrapper). Entries live in Boilerplate's own client-synced registries (see
 * [net.kernelpanicsoft.boilerplate.registry.Registrars]) rather than a hardcoded enum, so a new
...
abstract class PipeAttachmentType<S : AttachmentHolderState> : ItemLike
{
	 ...
	 * by appending `_part`; the block itself is registered automatically for Boilerplate's own
	 * types (see [net.kernelpanicsoft.boilerplate.registry.BlockRegistry]), and an addon type
	 ...
	 * [net.kernelpanicsoft.boilerplate.pipe.block.MultipartContentsLootFunction] against the
