 ...
 * [net.kernelpanicsoft.boilerplate.power.network.PressurePipeNetworkManager] (pressure pipes).
 *
...
abstract class AbstractPipeNetworkManager<N : AbstractPipeNetwork> {
	 ...
	 * [net.kernelpanicsoft.boilerplate.power.network.PressurePipeNetworkManager]) can layer its
	 ...
	 * `TickEvent.SERVER_LEVEL_POST` (see `Boilerplate.initCommon`), strictly *after* every loaded
	 * member's own block entity has already ticked (and so already called [ensureRegistered] on
	 * itself, see [net.kernelpanicsoft.boilerplate.pipe.entity.PipeBlockEntity.tick]) - so by the
