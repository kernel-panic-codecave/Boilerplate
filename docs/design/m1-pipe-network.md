...
- **Rendering**: each hook's baked model is authored once facing north and rotated onto the actual attached face by `PipeHookBlockEntityRenderer` at render time (`PoseStack` rotation around the block center) - one model per hook type serves all six faces. The model id is a *convention*, not a lookup table: `mymod:extraction` (the hook type id) resolves to the `inventory` variant of `mymod:extraction_hook`'s baked item model - the same model already guaranteed to exist for that hook's `HookItem` icon (registered as `<hook type path>_hook`), rather than a standalone block model nothing is guaranteed to have baked. An addon's own hook type gets a working model reference for free as long as its item follows the naming convention. Currently placeholder cuboid geometry/flat-color textures; the pipe body itself is a vanilla `"multipart"` blockstate model keyed to the same six connection booleans. Hand-written JSON for now - datagen is a planned follow-up, not yet implemented. Rendered via `ModelBlockRenderer.tesselateBlock` (real per-face block/sky light + Minecraft's directional shading), not the deprecated flat `renderModel` overload, which paints every quad with one uniform light value and no shading at all (reads as fullbright).
- **Hook presence is also real blockstate**, not just `hooks` block-entity data: `PipeBlock.hookPropertiesByDirection` mirrors `propertiesByDirection` (one `hook_<direction>` boolean per face), flipped alongside `hooks` in `useItemOn`/`useWithoutItem`. The multipart blockstate model needs this to skip a face's plain `pipe_arm` where a hook is attached instead - block-entity data isn't visible to a `"multipart"` blockstate JSON's `"when"` predicates at all, only real blockstate properties are.
- **Sync**: attaching/removing a hook (`PipeBlock.useItemOn`/`useWithoutItem`) explicitly calls `level.sendBlockUpdated(...)` after mutating `hooks` - the `NBTHolder` field's own `setChanged()` only marks the chunk dirty for saving, it never pushes a network resync, so without this a client's `hooks` (and the renderer reading it) never advances past whatever it was at the last chunk load.
...
- Two Archie `@Sync` bugs found while building this, now fixed upstream in Archie: (1) `NBTHolder.mapField`/`listField` registered the wrong serializer for `@Sync` (element instead of collection), crashing on sync; (2) `BlockEntityStateContainer`'s reflective property scan keyed by the raw Kotlin property name while `NBTHolder`'s manual registration/dirty-tracking keyed by `property.name.toSnakeCase()`, so a multi-word `@Sync` scalar field (e.g. `routingNorth`) also silently desynced. `PipeBlockEntity.routing: FaceRouting` (a single, single-word field) still avoids both rather than folding back into a `@Sync`-annotated `hooks` map - a mechanical follow-up cleanup, not required for correctness.
- `hooks` itself is deliberately *not* `@Sync` - attaching/removing a hook instead calls `level.sendBlockUpdated(...)` directly at the point of mutation (see `PipeBlock.useItemOn`/`useWithoutItem`), since `@Sync`/`BlockEntityStateManager` only pushes to players with a menu open, not to every nearby client's own block entity copy (what `PipeHookBlockEntityRenderer` actually reads).
