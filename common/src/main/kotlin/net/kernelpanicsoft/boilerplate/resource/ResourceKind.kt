...
/**
 * A kind of carrier resource that [net.kernelpanicsoft.boilerplate.resource.ResourceStackSerializer] can put on the wire - the item and
 ...
 * rather than a hardcoded case in [net.kernelpanicsoft.boilerplate.resource.ResourceStackSerializer]'s dispatch, so a new kind is just
 ...
 * through.
 *
 * [kindTag] is the leading string tag [net.kernelpanicsoft.boilerplate.resource.ResourceStackSerializer] writes before a stack's resource
...
abstract class ResourceKind {
	/** This kind's own unique wire tag - what [net.kernelpanicsoft.boilerplate.resource.ResourceStackSerializer] keys a stack's resource on. */
	...
	/**
	 * How this kind's amounts are counted - see [ResourceMeasure], which says what each case means
	 * and what it implies.
	 *
	 * Declared rather than defaulted: it is the one thing about a kind that nothing else can be
	 * inferred from and everything amount-shaped is inferred *from*, so a kind that has not said
	 * which it is has not finished describing itself.
	 */
	abstract val measure: ResourceMeasure

	/** The serializer for a resource of this kind, typed up to the generic [ResourceComponent] surface [net.kernelpanicsoft.boilerplate.resource.ResourceStackSerializer] works in terms of. */
	 ...
	 * Always reached through [ResourceIdentity.Companion.of] rather than called directly.
	 */
	open fun identityOf(resource: ResourceComponent): Any = resource

	/**
	 * [identityOf] with whatever varies *within* one resource stripped off - the item without its
	 * data components, the fluid without its data patch.
	 *
	 * What "the same thing, loosely" means is per-kind knowledge, and this is where a kind says it:
	 * an enchanted diamond pickaxe and a plain one are the same *item* and different resources, and
	 * only [ItemKind] knows which half of its identity is which. Asked by
	 * [net.kernelpanicsoft.boilerplate.pipe.hook.filter.ResourceConditionType] to answer its
	 * "match components" toggle for every kind at once, rather than that card carrying a branch per
	 * kind and having nothing to say about a kind an addon registers.
	 *
	 * Defaults to [identityOf], which is right for any kind whose resources carry nothing beyond
	 * what identifies them - a chemical, say. Such a kind's loose and exact matches are the same
	 * question, and the toggle simply makes no difference to it.
	 */
	open fun baseIdentityOf(resource: ResourceComponent): Any = identityOf(resource)
	...
	/** How far one unmodified scroll notch moves an authored amount of this kind - one item, or 100mB. */
	open val authoredStep: Long get() = 1L

	/**
	 * What [authoredStep] becomes with a modifier key held - the rest of this kind's scroll ladder.
	 *
	 * Defaults to whichever ladder this kind's own [measure] calls for, so a kind gets a usable one
	 * without declaring anything: a decimal ladder around the notch for a [ResourceMeasure.UNIT] or
	 * [ResourceMeasure.SCALAR] kind, whose notch sits in the middle of its range (10 / 1000 / 1 mB
	 * around a fluid's 100mB), and a ladder of whole multiples for a [ResourceMeasure.DISCRETE] one,
	 * whose notch is the smallest amount it has and can only run upward (4 / 16 / 64 items - a stack
	 * and its quarters, since nobody asks for ten of something the way they ask for a stack).
	 */
	open val scrollSteps: ScrollSteps
		get() = when (measure) {
			ResourceMeasure.DISCRETE -> ScrollSteps(
				shift = authoredStep * 16L,
				control = authoredStep * 64L,
				shiftAndControl = authoredStep * 4L,
			)

			ResourceMeasure.UNIT, ResourceMeasure.SCALAR -> ScrollSteps(
				shift = authoredStep / 10L,
				control = authoredStep * 10L,
				shiftAndControl = authoredStep / 100L,
			)
		}
	...
	/**
	 * How much of [resource], in this kind's authored unit, counts as **one whole** of it - a bucket
	 * of a fluid, a full stack of an item.
	 *
	 * The common denominator between kinds that are otherwise incomparable: 64 cobblestone, 16 ender
	 * pearls and 1000mB of water are each one whole, so a storage whose capacity is a single shared
	 * pool ([net.kernelpanicsoft.boilerplate.warehouse.rack.PooledResourceStorage]) can charge them
	 * all against the same figure and mean something by it. Nothing else can: a count and a
	 * millibucket are not the same size of thing, and treating them as one makes a bucket of water
	 * cost the same as a single stick.
	 *
	 * Per-*resource*, not per-kind, because for a counted kind the whole is that resource's own stack
	 * size - an ender pearl's whole is 16 of them, and a shulker box's is one.
	 *
	 * @param resource the resource being measured; ignored by a kind whose whole is fixed.
	 * @return one whole of [resource] in this kind's authored unit, at least `1`.
	 */
	open fun wholeAuthored(resource: ResourceComponent): Long = defaultAuthored
	 ...
	 * [ItemStack] and nothing else, so a kind that is not an item form
	...
	/**
	 * [amount] of [resource] as the vanilla [ItemStack] a crafting grid
	 ...
	 * registry a [TagKey] would need to be built against.
	 */
	abstract fun tagsOf(resource: ResourceComponent): List<TagKey<*>>
}

/**
 * The three modified rungs of a kind's scroll ladder - what one notch moves with Shift, Ctrl, or
 * both held. The unmodified rung is the kind's own [ResourceKind.authoredStep].
 *
 * Nothing here may be finer than the kind's smallest unit; a rung that computes below `1` is
 * clamped to it, which is how a kind that declares no ladder of its own still behaves.
 */
data class ScrollSteps(val shift: Long, val control: Long, val shiftAndControl: Long)
