...
- **`RequesterHookType`** - flips this hook's whole role from "request for myself" to "supply the interface" - `tryRequest` detects a boundary partner via `SubnetBoundary.interfaceAt` and, if found, iterates every `InterfaceHookState.ghosts` column computing its own shortfall (the ghost's count minus what's in the matching stock slot) and issuing one `RequestFulfillment.request` per shortfall, instead of reading its single `RequesterHookState.request` order. The interface *also* self-requests the same ghost shortfall on its own (see the type's own section above), so both paths chase the exact same target rather than a requester's full-stack guess - an over-supply would otherwise oscillate with `drainExcess` pushing the difference back out.
