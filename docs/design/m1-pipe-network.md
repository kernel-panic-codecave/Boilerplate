...
`TubularStorageBlockStateProvider.tubularStorageBlockStates()` generates every blockstate/block-model/item-model JSON under `assets/tubularstorage` (`pipe`/`glass_pipe`/`hook`'s blockstates, `pipe_core`/`pipe_arm`/`glass_pipe_core`/`glass_pipe_arm`/`extraction_hook`/`sorting_hook`'s block models, and the four item models pointing at them) - hand-written JSON is gone. `TubularStorageDatagen : ADatagenEventObject` registers it, gated behind `ADataGeneratorPlatform.isDataGen` in `TubularStorage.init()` exactly like `TubularStorageGameTest`/`AGameTestPlatform.isGameTest` - `archie-datagen-common` is the same story as `archie-gametest-common`, a dev-only dependency absent from the production runtime classpath.

Output goes to `common/src/main/generated`, wired in as an extra resources root on `common`'s own sourceSet (`fabric`/`neoforge`'s existing `processResources { from(project(":tubularstorage-common").sourceSets.main.get().resources) }` merge picks it up for free) - not `common/src/main/resources` directly, despite that being where the hand-written JSON used to live. Minecraft's datagen `CachedOutput` treats its entire output directory as exclusively its own and deletes anything in it a provider didn't just (re)write; pointing it straight at `src/main/resources` deleted the hand-placed textures and the gametest structure NBT sitting alongside the JSON on the first real run. `common/src/main/generated` is committed (a `.gitignore` exception carved out of the `**/src/main/generated` rule that's otherwise right for Archie, a library regenerated on demand by consumers) - unlike Archie, a TubularStorage checkout needs to build without a `runDatagen` step first. Regenerate with `./gradlew :tubularstorage-fabric:runDatagen` (or the `neoforge` variant) after changing block/model logic, and commit the result.
