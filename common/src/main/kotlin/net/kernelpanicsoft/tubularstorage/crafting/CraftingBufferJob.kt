 ...
 * tradeoff [net.kernelpanicsoft.tubularstorage.pipe.entity.TravelingItem] already has;
...
class CraftingBufferJob(val id: String, val target: ItemResource, val targetAmount: Long, val steps: List<CraftStep>) {
	...
	/** The pattern-provider hook's own position for each [steps] index - where a `CRAFTING`-kind step's own buffer delivery needs to land, see [net.kernelpanicsoft.tubularstorage.pipe.hook.PatternProviderHookType]. */
